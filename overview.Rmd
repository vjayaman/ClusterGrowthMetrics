...
renderPlotly({
  ...
  tmp1 <- b1 %>% select(id, tp1_h, tp1_cl, tp1_cl_size) %>% add_column("TP" = "TP1", .after = 1) %>% 
    set_colnames(c("Rows (from previous tab)", "TP", "height", "cluster", "Cluster size"))
  tmp2 <- b1 %>% select(id, tp2_h, tp2_cl, tp2_cl_size) %>% add_column("TP" = "TP2", .after = 1) %>% 
    set_colnames(c("Rows (from previous tab)", "TP", "height", "cluster", "Cluster size"))
  
  b5 <- b4 %>% left_join(., bind_rows(tmp1, tmp2), by = c("Rows (from previous tab)", "TP", "Cluster size"))
