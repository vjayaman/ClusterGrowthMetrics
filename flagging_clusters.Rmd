...
precomp <- clustComp(time2_coded, h, "h_before")
...
just_originals <- clusterAssignments(h, time2_raw, precomp$h_before, new_isolates)

single_height <- resultsProcess(h, time2_raw, precomp$h_before, df1, ids, just_originals)

ids <- c(unlist(ids), single_height$flagged %>% unique()) %>% unique()
metrics <- addToMetrics(height, ids)
saveRDS(single_height, paste0("tmpheights/h_", height, ".Rds"))

if (isTRUE(save_data)) {
  saveRDS(single_height, paste0("height_data/h_", height, ".Rds"))
}else {
  message("Note: not saving data; testing")
}
```
...
for (j in 1:length(colnames(time2_raw)[-1][-1])) {
  ...
  print(paste0(j, "/", length(colnames(time2_raw)[-1][-1]), ", height ", height2))
  ...
  inds_new_clusters <- setdiff(postcomp$composition, precomp$composition)
  if (length(inds_new_clusters) > 0) {stop("Try this one!")}
  b4 <- postcomp %>% filter(composition %in% inds_new_clusters)  
  ...
  stayed_the_same <- postcomp %>% 
    filter(!(composition %in% inds_new_clusters)) %>% 
    left_join(., precomp, by = "composition") %>% 
    select(-composition)

  tmp1 <- inner_join(single_height, stayed_the_same, by = c("tp2_cl" = "h_before")) %>% 
    select(-tp2_cl) %>% rename(tp2_cl = h_after) %>% 
    select(colnames(single_height))
  tmp1$tp2_h <- height2
  
  if (any(tmp1$tp2_h != height2)) {stop("TP2 height not labelled correctly")}
  ...
  if (nrow(b4) > 0) {
    ...
    just_originals <- clusterAssign(height2, time2_raw, b4$h_after, new_isolates)
    single_height2 <- resultsProcess(height2, time2_raw, b4$h_after, df1, ids, just_originals)
    if (isTRUE(save_data)) {
      saveRDS(single_height2, paste0("changing_clusters/h_", height2, ".Rds"))
    }
    ids <- c(ids, single_height2$flagged) %>% unique()
    single_height <- single_height2 %>% bind_rows(tmp1, .)
  }else {
    single_height <- tmp1
  }
  if (isTRUE(save_data)) {
...
all_sizes <- time2_raw %>% meltData(., "isolate") %>% 
  select(-isolate) %>% countCases(., "tp2_cl_size") %>% 
  set_colnames(c("tp2_h", "tp2_cl", "tp2_cl_size")) %>% 
  factorToInt(., "tp2_h")
...
dne <- novel_df %>% 
  ...
  factorToInt(., "tp2_h") %>% 
  add_column(tp1_h = "DNE", tp1_cl = "DNE", tp1_cl_size = 0, flagged = NA) %>% 
  select(isolate, tp1_h, tp1_cl, tp1_cl_size, tp2_h, tp2_cl, tp2_cl_size, flagged)

need_to_track <- original_tracking %>% 
  dplyr::filter(id %in% comp_sizes$id[comp_sizes$dif > 0]) %>% 
  dplyr::select(-isolate) %>% unique()

using_originals <- novel_df %>% 
  right_join(., need_to_track, by = c("tp2_h", "tp2_cl", "id")) %>% 
  dplyr::select(isolate, tp1_h, tp1_cl, tp1_cl_size, 
                tp2_h, tp2_cl, tp2_cl_size, flagged) %>% 
  mutate(across(c(tp1_h, tp1_cl), as.character))
...
metrics <- final_df %>% arrange(., -fold_change) %>% 
