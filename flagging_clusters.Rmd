...
saveRDS(stopwatch, "stopwatch.Rds")
...
# actual_clusters <- checkingClusters(time2_raw, 1)
# 
# # definitely have originals and might have novels
# omn <- time2_raw %>% filter(!(isolate %in% novels)) %>% checkingClusters(., 1)
# omn_ids <- omn$id %>% unique() %>% sort()
# 
# # definitely have novels and might have originals
# nmo <- time2_raw %>% filter(isolate %in% novels) %>% checkingClusters(., 1)
...
tmp <- checkingClusters(time2_raw)
...
cwn <- tmp %>% filter(isolate %in% novels)
# non-novel cluster assignments
cnn <- tmp %>% filter(!(isolate %in% novels))
...
actual_clusters <- time2_raw %>% checkingClusters(., 1)
# 1 800 684
