...
clusters_one <- lapply(1:length(a1_clusters), function(i) {
  indices <- which(a1$tp2_cl == a1_clusters[i])
  a1$isolate[indices] %>% sort() %>% 
    paste0(collapse = ",") %>% return()
}) %>% unlist() %>% 
  tibble(composition = ., h_bef = a1_clusters)
...
just_originals <- clusterAssignments(height, time2_raw, clusters_one$h_bef, new_isolates)

single_height <- resultsProcess(height, time2_raw, clusters_one$h_bef, df1, ids, just_originals)
...
for (j in 1:length(colnames(time2_raw)[-1][-1])) {
  ...
  one_height <- time2_coded %>% 
    select(isolate, all_of(height2)) %>% 
    set_colnames(c("isolates", "height2")) %>% 
    group_by(height2)
  new_clusters <- one_height$height2 %>% unique()
  
  # these are all the clusters and their composition
  clusters_two <- lapply(1:length(new_clusters), function(i) {
  ...
  inds_new_clusters <- setdiff(clusters_two$composition, clusters_one$composition)
  b4 <- clusters_two %>% dplyr::filter(composition %in% inds_new_clusters)  
  ...
  stayed_the_same <- clusters_two %>% 
    dplyr::filter(!(composition %in% inds_new_clusters)) %>% 
    dplyr::left_join(., clusters_one, by = "composition") %>% 
  ...
  clusters_one <- clusters_two %>% set_colnames(c("composition", "h_bef"))
