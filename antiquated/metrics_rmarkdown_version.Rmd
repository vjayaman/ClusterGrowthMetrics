...
params:
  data1: data/timepoint1_data.csv
  data2: data/timepoint2_data.csv
---
...
if (file.exists(params$data1)) {
  time1 <- read.csv(file = params$data1, stringsAsFactors = FALSE, numerals = "no.loss", check.names = FALSE) %>% as_tibble()
}else {
...
if (file.exists(params$data2)) {
  time2 <- read.csv(file = params$data2, stringsAsFactors = FALSE, numerals = "no.loss", check.names = FALSE) %>% as_tibble()
}else {
...
#   - merging using isolate as an identifier
