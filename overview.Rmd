...
x <- c("ggplot2", "plotly", "flexdashboard", "RColorBrewer")
library(shinyjs)
...
```{r}
# DT::renderDataTable({clusters[1:10,]})
```
...
```{r}
renderPlotly({
  min_size <- 10#input$fc_violin_ms
...
selectInput("flagged_clusters", "Select a TP1 originating cluster to track through TP2:", choices = df$tp1_id %>% unique())
```
...
# DT::datatable(flagged_w_iso, options = list(columnDefs = list(list(className = "dt-center", targets = "_all")),
#                                      dom = "ti", pageLength = nrow(flagged_w_iso), scrollY = "500px"),
